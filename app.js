const state={data:null,editingId:null,busy:false};
...
function refreshData(){
  if(!configReady()) return openSettings(true);
  setBusy(true); $('syncText').textContent='Syncing…';
  jsonp('data').then(result=>{
    if(!result.ok) throw new Error(result.error||'Unable to sync.');
...
function renderDashboard(){
  ...
  $('dashboardCards').innerHTML=s.cards.map(cardHTML).join('');
}
function renderCards(){$('allCards').innerHTML=state.data.summary.cards.map(cardHTML).join('')}
function cardHTML(c){
  const cls=String(c.status).toLowerCase().replace(/\s+/g,'-');
  return `<article class="ccard"><div class="ccard-top"><div><h3>${esc(c.name)}</h3><span class="due">Due day ${esc(c.dueDay)}</span></div><span class="status ${esc(c.status).replace(/\s/g,'-')}">${esc(c.status)}</span></div>
...
function renderTransactions(){
  ...
  const rows=state.data.transactions.filter(t=>!q||[t.date,t.card,t.description,t.category,t.type,t.notes].join(' ').toLowerCase().includes(q));
...
async function submitTransaction(e){
  ...
  setBusy(true);
  try{await postAction(state.editingId?'update':'add',tx);toast(state.editingId?'Transaction updated.':'Transaction saved.');resetForm();await refreshData()}
  catch(err){toast(err.message||'Unable to save.')}finally{setBusy(false)}
}
function editTx(id){
